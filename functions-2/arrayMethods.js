...
const filteredArr = mixedNumbers.filter(Element => Element % 2 === 0)
...
const afterTax = prices.map(num => num * 1.07)
...
/*
  Use a higher order method to get all the order totals after adding in 
  the sales tax (given to you as a tax rate, hint: you'll need to do some multiplication). 
  Your answer should be an array of numbers, one total for each order.
*/

// CODE HERE

const orderTotals = orders.map(orders => num.price * num.tax)
...
// CODE HERE // ask for help
const bobsTotal = purchases.reduce((purchases, acc) => purchases["owner"]["bob"]["price"] + acc )